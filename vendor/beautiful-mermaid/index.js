...
// --- CJK / double-width support (patched for terminal alignment) ---
// Terminal display width: most CJK and full-width chars occupy 2 columns.
// The original engine assumed 1 char === 1 cell, which misaligns boxes/edges
// when labels contain CJK characters. These helpers + a sentinel continuation
// cell fix that without changing the canvas model.
var SENTINEL = "\0";
function charWidth(ch) {
  const cp = ch.codePointAt(0);
  if (cp === void 0) return 1;
  // Zero-width / combining chars
  if (cp === 0) return 0;
  // CJK Unified, CJK Ext A, CJK Compatibility, Hiragana, Katakana, CJK symbols,
  // Fullwidth/Halfwidth forms, Hangul Syllables
  if (cp >= 0x1100 && (cp <= 0x115f || cp >= 0x2e80 && cp <= 0x303e || cp >= 0x3041 && cp <= 0x33ff || cp >= 0x3400 && cp <= 0x4dbf || cp >= 0x4e00 && cp <= 0x9fff || cp >= 0xa000 && cp <= 0xa4cf || cp >= 0xac00 && cp <= 0xd7a3 || cp >= 0xf900 && cp <= 0xfaff || cp >= 0xfe30 && cp <= 0xfe6f || cp >= 0xff00 && cp <= 0xff60 || cp >= 0xffe0 && cp <= 0xffe6 || cp >= 0x20000 && cp <= 0x3fffd)) {
    return 2;
  }
  return 1;
}
...
// Write a string into the canvas starting at (x, y), advancing by each char's
// display width. Double-width chars also mark their continuation cell with a
// SENTINEL so canvasToString can skip it (the terminal already renders the
// single codepoint across 2 columns).
function placeStr(canvas, x, y, str, maxX) {
  let cx = x;
  for (const ch of str) {
    if (maxX !== void 0 && cx >= maxX) break;
    if (canvas[cx]) canvas[cx][y] = ch;
    const w = charWidth(ch);
    if (w === 2 && canvas[cx + 1]) canvas[cx + 1][y] = SENTINEL;
    cx += w;
  }
}
...
function drawText(canvas, start, text) {
  ...
  placeStr(canvas, start.x, start.y, text);
}
...
function drawBox(node, graph) {
  ...
  placeStr(box, textX, textY, label);
  return box;
}
function drawMultiBox(sections, useAscii, padding = 1) {
  ...
  for (let s = 0; s < sections.length; s++) {
    ...
    for (const line of lines) {
      ...
      placeStr(canvas, startX, row, line, boxWidth - 1);
      row++;
    }
...
function drawSubgraphLabel(sg, graph) {
  ...
  placeStr(canvas, labelX, labelY, sg.name, width - 1);
...
function renderSequenceAscii(text, config) {
  ...
  function drawActorBox(cx, topY, label) {
    ...
    placeStr(canvas, ls, topY + 1, label);
  ...
  for (let m = 0; m < diagram.messages.length; m++) {
    ...
    if (isSelf) {
      ...
      placeStr(canvas, labelX, y0 + 1, msg.label, totalW);
      ...
      placeStr(canvas, labelStart, labelY, msg.label, totalW);
      if (leftToRight) {
  ...
  for (let b = 0; b < diagram.blocks.length; b++) {
    ...
    placeStr(canvas, bLeft + 1, topY, hdrLabel, bRight);
    ...
    for (let d = 0; d < block.dividers.length; d++) {
      ...
      if (dLabel) {
        const dStr = `[${dLabel}]`;
        placeStr(canvas, bLeft + 1, dY, dStr, bRight);
      }
    }
  }
  for (const np of notePositions) {
    ...
    for (let l = 0; l < np.lines.length; l++) {
      ...
      placeStr(canvas, np.x + 2, ly, np.lines[l]);
    }
...
function renderClassAscii(text, config) {
  ...
  for (const rel of diagram.relationships) {
    ...
    if (rel.label) {
      ...
      placeStr(canvas, labelStart, midY, paddedLabel, totalW);
    }
  }
  return canvasToString(canvas);
}
  ...
  for (const rel of diagram.relationships) {
    ...
    if (sameRow) {
      ...
      if (rel.label) {
        ...
        if (labelY >= 0) {
          placeStr(canvas, labelStart, labelY, rel.label, endX + 1);
        }
      }
    } else {
      ...
      if (rel.label) {
        ...
        if (midY >= 0) {
          increaseSize(canvas, labelX + displayWidth(rel.label), midY);
          placeStr(canvas, labelX, midY, rel.label);
        }
      }
    }
  }
